...
public class ReflectanceBinLUT {
    ...
    ReflectanceBinLUT(String lutPath, int aerosolModelID, float[] merisWvl, float[] aatsrWvl) throws OperatorException {
        float lutWvl;
        int iWvl;
    ...
    public void subsecLUT(String instr, float pres, float vza, float vaa, float sza, float saa, float[] wvl, float[][][] a) {
        int nWl = wvl.length;
 ...
 */
            ...
            throw new OperatorException(mess, ex);
        } catch (ClassNotFoundException ex) {
            String mess = "Could not read LUT file:\n" + ex.getMessage();
            Logger.getLogger(ReflectanceBinLUT.class.getName()).log(Level.SEVERE, mess, ex);
            throw new OperatorException(mess, ex);
        } finally {
            try {
