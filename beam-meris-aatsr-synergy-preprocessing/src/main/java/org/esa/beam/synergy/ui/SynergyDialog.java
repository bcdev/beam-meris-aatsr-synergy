 ...
 */
public class SynergyDialog extends SingleTargetProductDialog {
    ...
    private SynergyForm form;
    private SynergyModel model;

    public static SingleTargetProductDialog createDefaultDialog(String operatorName, AppContext appContext) {
        return new SynergyDialog(operatorName, appContext, operatorName, null);
    }

    public SynergyDialog(String operatorName, AppContext appContext, String title, String helpID) {
...
//        ioParametersPanel.add(tableLayout.createVerticalSpacer());
        ...
        this.model = new SynergyModel();
        this.form = new SynergyForm(appContext, operatorSpi, model, getTargetProductSelector());

        final JPanel processingParametersPanel = form.createProcessingParametersPanel();
//        processingParametersPanel.setBorder(new EmptyBorder(4, 4, 4, 4));
//        this.form.add("Processing Parameters", new JScrollPane(processingParametersPanel));
        ioParametersPanel.add(processingParametersPanel);
        ioParametersPanel.add(tableLayout.createVerticalSpacer());
...
//            this.form.add("Processing Parameters", new JScrollPane(paremetersPanel));
        ...
        final JPanel cloudScreeningParametersPanel = form.createCloudScreeningParametersPanel();
        cloudScreeningParametersPanel.setBorder(new EmptyBorder(4, 4, 4, 4));
        this.form.add("Cloud Screening", new JScrollPane(cloudScreeningParametersPanel));

        final JPanel aerosolParametersPanel = form.createAerosolParametersPanel();
        aerosolParametersPanel.setBorder(new EmptyBorder(4, 4, 4, 4));
        this.form.add("Aerosol Retrieval and Atmospheric Correction", new JScrollPane(aerosolParametersPanel));
